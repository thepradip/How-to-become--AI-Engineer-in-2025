"""Fetch the real Ames House Prices dataset into ./data (OpenML 'house_prices')."""
...
def fetch() -> pathlib.Path:
    ...
    try:
        ...
        frame = fetch_openml("house_prices", as_frame=True).frame
        frame.to_csv(CSV, index=False)
        print(f"✓ downloaded real Ames data → {CSV} ({len(frame)} rows)")
